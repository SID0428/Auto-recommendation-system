...
Set `VITE_GROQ_API_KEY` in `.env` before starting the dev server.
...
Gemini is disabled by default. To switch back later, set `VITE_AI_PROVIDER=gemini` and provide `VITE_GEMINI_API_KEY`.
...
3. `aiService.js` builds a structured prompt with the full 20-product catalog and sends it to Groq through the Vite proxy.
4. The AI returns JSON: `{ summary, recommended: [{id, reason}] }`. If the API is unavailable, a local fallback ranks the catalog from the same user preference.
5. App highlights matched products (green border, floated to top) and shows per-card AI reasons.
6. User can further filter by category using the filter bar.
